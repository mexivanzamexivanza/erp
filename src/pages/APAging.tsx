  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
        <div>
          <h1 className="pageTitle">AP aging</h1>
          <div className="pageSub">Managers/Admins only. Open bill balances grouped by days past due.</div>
        </div>
      ...
      <section className="card" style={{ padding: 14 }}>
        <div style={{ fontWeight: 800, marginBottom: 10 }}>Bills (open only)</div>

        {loading ? (
          <div style={{ color: "var(--muted)" }}>Loading...</div>
        ) : openOnly.length === 0 ? (
          <div style={{ color: "var(--muted)" }}>No open payables.</div>
        ) : (
              ...
              <tbody>
                {openOnly.map((r) => (
                  <tr key={r.bill_id}>
                    ...
                    <td>{new Date(r.billed_at).toLocaleDateString()}</td>
